...
HEADLINES_PATH = os.path.join(settings.BASE_DIR, 'game\\questions.json')
...
def headline_get_question(request):
    ...
    try:
        if question_id:
            ...
            for q in questions:
                if q["fields"]["id"] == int(question_id) + 1:
                    question = q["fields"]
            if not question:
...
def headline_submit_answer(request):
    ...
    try:
        ...
        for q in questions:
            if q["fields"]["id"] == question_id:
                question = q

        if not question:
            return Response({'error': 'No more questions'}, status=404)
        else:
            correct_answer = question['fields']['correct_headline']
            print("sel: " + selected_headline)
            print("ans: " + correct_answer)
            if selected_headline == correct_answer:
